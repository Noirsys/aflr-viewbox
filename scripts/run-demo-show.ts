 ...
 * don’t already exist) and drives the viewbox via WebSocket messages. It sends
...
async function seedDemoAssets(): Promise<void> {
  ...
  await writeIfMissing(
    ...
    svgCard({
      ...
      subtitle: "Multiple units dispatched • Stand by",
      bg: "#4c1d95",
    })
  );
  ...
  await writeIfMissing(
    ...
    [
      "BREAKING: Scanner reports increased activity • Updates pending",
      "TRAFFIC: Slowdowns near downtown • Use alternate routes",
      "COURTS: Arraignments scheduled 9:00 AM • Public docket posted",
      "WEATHER: Cold front moving in overnight • Stay alert",
    ].join("\n")
  );
  ...
  await writeIfMissing(
    ...
    [
      "EMERGENCY: Severe Weather Warning • Seek shelter if necessary",
      "PUBLIC SAFETY: Avoid restricted areas • Follow official guidance",
      "UPDATES: More details as confirmed • Stand by",
    ].join("\n")
  );

  console.log("✅ Seeded demo assets into public/media/** (if missing)");
}
...
runDemo().catch((err) => {
  console.error("❌ Demo show failed:", err);
  process.exitCode = 1;
});
